...
Partial Class ShowPayin_Sum_CQ
    ...
    'Dim strConn As String = WebConfigurationManager.ConnectionStrings("RECONCILE").ConnectionString.ToString
    ...
    Private Sub ShowGrid()
        Try
            ...
            DT = C.GetDataTableReconcile(strSql, True)
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim Receipt As Double = e.Row.Cells(4).Text
            'Dim Payin As Double = e.Row.Cells(5).Text
            'Dim cq_in_sys As Double = e.Row.Cells(6).Text

            Receipt = String.Format(e.Row.Cells(4).Text, "{0:00,00.00}")
            e.Row.Cells(4).Text = Format(Receipt, "#,##0.00")
            e.Row.Cells(4).HorizontalAlign = HorizontalAlign.Right
            e.Row.Cells(4).Width = "200"

            'Payin = String.Format(e.Row.Cells(5).Text, "{0:00,00.00}")
            'e.Row.Cells(5).Text = Format(Payin, "#,##0.00")
            'e.Row.Cells(5).HorizontalAlign = HorizontalAlign.Right


            'cq_in_sys = String.Format(e.Row.Cells(6).Text, "{0:00,00.00}")
            'e.Row.Cells(6).Text = Format(cq_in_sys, "#,##0.00")
            'e.Row.Cells(6).HorizontalAlign = HorizontalAlign.Right

            'Receipt = Math.Round(Receipt, 2).ToString
            'Payin = Math.Round(Payin, 2).ToString
            'cq_in_sys = Math.Round(cq_in_sys, 2).ToString
            'e.Row.Cells(4).Text = Receipt
            'e.Row.Cells(5).Text = Payin
            'e.Row.Cells(6).Text = cq_in_sys
        End If
    End Sub
End Class
